...
function renderCart(){
  ...
  // Clear Cart handler with confirm + alert (for screenshots)
  $("#clearCart")?.addEventListener("click", () => {
  ...
  // Process Order handler (shows 'already processed' on second click)
  $("#processOrder")?.addEventListener("click", () => {
    // 1) If already processed in this tab/session, show message first
    if (sessionStorage.getItem(ORDER_KEY) === "1") {
      alert("Your order has already been processed. Thank you!");
      return;
    }

    // 2) Require something to process: either cart items or a saved custom order
    const hasCart = getCart().length > 0;
    const hasCustom = !!sessionStorage.getItem(CUSTOM_KEY);
    if (!hasCart && !hasCustom) {
      alert("Your cart is empty. Add items or save a custom order first.");
      return;
    }

    // 3) Simulate processing: mark processed, clear data, update UI, alert success
